...
/**
 * Stream JSONL records whose raw line contains any of the needles
 * (case-insensitive prefilter — avoids parsing every object on large corpora).
 */
export async function* streamJsonlObjectsMatching(
  ...
  const upperNeedles = needles
    ...
    .filter((n) => n.length >= 2);
  if (upperNeedles.length === 0) {
    yield* streamJsonlObjects(absolutePath);
    return;
  }
  const rl = createInterface({
    input: createReadStream(absolutePath, { encoding: "utf8" }),
